...
class GETResponse extends GETRequest {
   ...
   * 回复流
   */
  final val Out_stream: BufferedOutputStream = new BufferedOutputStream(Server_acc.getOutputStream)

  /**
   * 回复HTTP
   */
  def response_Http(): Unit = zhaoSocket.responseData(toString, Out_stream, `isHTTP?` = true)
