...
import config.{ConfigBase, WEB_ConfigBase}
import run_zhao.MAIN_zhao.{help, sc_sql}
 ...
 * @author zhao
 */
class VIEW_Manager extends project {
  /**
   * todo 数据路径与数据表 匹配器
   */
  ...
  override def T_[>>>](args: Array[String], dataset: Dataset[Array[>>>]])(implicit Con: Map[String, String]): Unit = {
    ...
    if (C_view && args.head == "create") {
      ...
      args match {
        case _ if C_view && `>= 2` && args.head == "select" => D_(null, args)
        case _ if `>= 2` && (args.head == "select" || args.head == "save") => D_(BackData, args)
  ...
  def createView(DataSrc_type_str: String, StructType_Schame: Array[String], args: Array[String]): Unit = {
    DataSrc_type_str match {
      ...
      case x => val strings: Array[String] = x.split(":")
        if (strings.length > 1) {
          D_(sc_sql.createDataFrame(
            UDFDATASourceFormat.UDFDATA_SOURCE_FORMAT_HASH_MAP.get(strings(1)).run(sc_sql), getSchame(StructType_Schame)
          ), args)
        } else {
  ...
  override def D_[>>>](dataset: Dataset[>>>], SQL: Array[String]): Unit = {
    var DataSnk_type_str: String = null
    var savePath: String = null
    if (SQL.head == "save") {
      DataSnk_type_str = SQL(2)
      savePath = SQL(3)
    }
    SQL match {
      case sql if sql.head == "create" && dataset != null =>
        dataset.createOrReplaceTempView(sql(3))
        dataset.printSchema()
        CON += (sql(3) -> sql(2))
        sc_sql.catalog.cacheTable(sql(3))
      case sql if sql.head == "select" && sql.contains("view") => println(
        if (CON.nonEmpty) {
          CON
      ...
      case sql if sql.head == "save" => saveMd(DataSnk_type_str, dataset, savePath)
      case _ =>
        new Thread(() => {
          try {
            val sql_lan = SQL.mkString(" ")
            BackData = sc_sql.sql(sql_lan)
            BackData.show(numRows = 124, truncate = false)
