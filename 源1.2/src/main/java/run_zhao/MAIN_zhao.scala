...
object MAIN_zhao extends Logging {
   ...
   */
  private def loadClassUDF(args: Array[String]): Unit = {
    val strings: Array[String] = Array("run_zhao.udf.DataTear") ++ (if (args.length >= 3) args(4).split(",+") else Array(""))

    /**
     * 初始化所有的数据解析自定义插件
     */
    for (classStr: String <- strings) {
      val value = Class.forName(classStr.trim)
      value.newInstance().asInstanceOf[UDFDATASourceFormat].Init()
    }
