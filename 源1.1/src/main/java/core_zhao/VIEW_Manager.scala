...
class VIEW_Manager extends project {
      ...
      case "MySQL" =>
        ...
        D_(
          sc_sql.read.format("jdbc")
            ...
            .load(), args)
      case "HBASE" => D_(
        sc_sql.createDataFrame(DataFromHbase.getDaraRDD_FromHBASE(args(2)).map(Row.fromSeq(_))
          , getSchame(StructType_Schame)), args)
  ...
  override def D_[>>>](dataset: Dataset[>>>], SQL: Array[String]): Unit = {
    ...
    SQL match {
      ...
      case _ =>
        val SQL_Thread = new Thread(() => {
          try {
            BackData = sc_sql.sql(SQL.mkString(" "))
