...
object DataFromHbase {
   ...
   */
  def getDaraRDD_FromHBASE(inpath: String): RDD[Array[String]] = {
    ...
    MAIN_zhao.sc_sql.sparkContext.newAPIHadoopRDD(
      ...
      x => {
        val array: ArrayBuffer[String] = ArrayBuffer(Bytes.toString(x._1.get(), x._1.getOffset, x._1.getLength)) // 这里用来盛放一行的数据
        // todo 下面将一行数据的所有单元格的值 直接添加到行数据容器中
        x._2.rawCells().foreach(cell => array.append(Bytes.toString(cell.getValueArray, cell.getValueOffset, cell.getValueLength)))
        array.toArray // 最后返回行数据组 里面存储的就是我们需要的 Array[String] 一个元素
      }
    )
  }
}
